...
- Source-repo fallback is only for a full source checkout when the integrated CLI is unavailable; do not use it from installed release skill bundles. In that case, run the matching Python module command from the repository root.
...
## Typical commands
- Prefer the integrated CLI. Use source-repo module commands only from a full source checkout when the binary/console entrypoint is unavailable.
...
- Source-repo fallback module paths mirror the resource script file names, but they are not supported from installed release skill bundles.
